    ...
    public abstract class DouyinBasicSyncJob : IJob, IDisposable
    {
            ...
            DouyinCookie cookie, Aweme item, VideoBitRate bitRate, string savePath, string coverSavePath, string avatorPath, List<DouyinMergeVideoDto> dynamicVideos = null, DouyinCollectCate cate = null, DouyinFollowed followed = null)
        {
            ...
            var video = new DouyinVideo
            {
                ...
                // ⚠️列表接口author的short_id/unique_id均恒为null(实测仅detail接口返回)——
                // 回落链:响应unique_id(可搜索的自定义号,2026-09-03优先)→followed参数/查dy_follow(已改存unique_id)→响应short_id数字号兜底
                AuthorDouyinNo = !string.IsNullOrWhiteSpace(item.Author?.UniqueId)
